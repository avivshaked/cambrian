...
**The floor is real (2026-09-06, commit `a268311`):** a static box collider with its top
face at y = −D, the restoring mirror below it retired for the shared branch, every
founder, inoculant and newborn raised until its bounding sphere clears the bed. The
20,000-s replay of `r25q-s2`'s settings on the new build diverged zero times against the
recorded three, with population and depth tracking within the wingspan. **One reading
corrected:** the ~1 m/s founding speed was never the floor — with the bed in it does not
move, and with the surface rule off it drops to 0.35 m/s; it is a buoyant founder drawn
...
**Ruled on 2026-09-10 evening**, after the agent's first look (logbook/0086), on the five
questions put in plain terms, "proceed with your recommendations": the dispersal radius
stays at 5 m for the base round; the box keeps its shape, to be tested on its own later;
the current's speed is 0.1 m/s, so that a swimmer can beat the water; founders and
inoculants reserve the adult as children do (`TryReserveFounder` takes the adult plan); the
plotting library for the positions reader may be installed (it could not be, this machine
refusing PyPI's certificate; the reader prints without it). Round 35 is the base round on
these values (logbook/0087).
