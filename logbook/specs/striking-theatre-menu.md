...
**2026-09-16**  ·  asked by the owner, and on the constraints as well.

> "put an opus sub agent to do some exploration and thinking how to make the theatre a lot
> more visually striking; it can plan usage of open source libraries, or leonardo, or even
> if the output justifies the cost, additional resources"
>
> "it could suggest alternatives to our decisions should that make sense"

An Opus subagent made the design pass, read-only, from the pictures first and the code
second. The agent's reading follows, and the report is below it as delivered, with its
judgements marked as readings of the pictures.

## The reading (the agent's)

The diagnosis is verified, and it is a defect rather than a matter of taste. The subagent
found, and I checked in the files, that the theatre renders with Unity's image chain mostly
off. The project is in gamma colour space (`ProjectSettings.asset`,
`m_ActiveColorSpace: 0`). The renderer has no post-processing data and no features
(`UniversalRenderer.asset`), and the pipeline has no volume profile and no anti-aliasing
(`UniversalRenderPipelineAsset.asset`, the default profile's `components: []`).

The snapshot's target is 8-bit. The key light is placed once from the fly camera's starting
rotation (`TheatreSkin.cs:362`, called once at `TheatreRunner.cs:219`), and never re-aimed.
So five of the six snapshot views and every flight around a body are lit by the fill.

The dark field was designed for values that sit in the bottom tenth of the range, and
nothing redistributes them. That is the whole of "can barely see anything", and it is a
day's work, hash-safe, before any question of style is asked.

The order is the one-day pass first, as the report says and for its reason. Every later
judgement is made from pictures, and the pictures are taken through a broken chain. Then
comes the one-week pass, with before-and-after pairs at each step so the owner's eye rules.
The ambitious version is the three specs already queued (timeline, checkpoints, safari)
plus the inherited skin genes, which is the owner's genome decision and stays a proposal.

On the reopened decisions, R3 (versioned framing) and R5 (hue in the film's chrome, never
the instrument's) cost nothing, and I would take both. R1 (a lit top layer) and R2 (guild
hue through backlit translucency) are the owner's. Both are the dark-field rule's edges,
and both are where the report reads the largest "alive" gain. R4 (a glow in film frames
only, markers in every cited picture) is safe as bounded.

D4, drawn eyes, I would refuse for now. A creature that has not evolved an eye should not
wear one. The world may yet evolve a light sense (round 40), and that organ can then be
drawn as what it is.

In the schedule, the one-day pass slots before the timeline (HANDOFF item 2a), since the
timeline's pictures should be taken through the fixed chain. The week pass folds into the
safari's build.

Leonardo costs nothing on the owner's Essentials plan, which makes the report's "target
frames for the look" the right first use of it. Generate what the tank should look like,
put it beside a close view, and turn the dials toward it, uploading nothing.

## The rulings (owner, 2026-09-16, in conversation)

1. **Grade everything**: every picture from here on is taken through the post stack, with
   the look version stamped into the frame's label. The pictures already taken stand.
...
4. **No drawn eyes**, and no organ the genome does not encode. The owner wondered about a
   mouth for the absorptive or a fin, and called it cheating. The agent's line was accepted
   as the plan. The absorptive tissue may have its own surface, since the part is the
   stomach by function, and a sense that evolves gets drawn as the organ it is.

5. **Inherited skin genes: wanted**, cost permitting. The theatre-only version comes first:
   the skin derived from the genome's own features, one day, no hash moves. The genome
   version follows only if the first is not enough. It means six to ten neutral numbers
   that mutate, a format bump, every stored genome re-extracted, and RNG draws that change
   every trajectory, so a round boundary.

Leonardo is a monthly token allowance rather than a free tool. The agent writes a prompt
...
The display chain is the first problem, ahead of the art. Four settings mean the theatre is
...
Without a tonemap a body's key-lit facets clip to white while everything else sits in four
or five distinguishable levels. That is what `r39big-m9-t600-bed.png` looks like: a grey
ramp you can only just find, the picture the owner said "can barely see anything" about.

The key light is aimed at a camera that is not the one rendering. `TheatreSkin.Apply` is
called once from `TheatreRunner.cs:219` with the fly camera. The key is placed at
`behind * Euler(38, -26, 0)`, the fly camera's starting rotation, and never re-aimed.

Fly to the other side of a body and the 0.55 fill lights it in place of the 2.1 key. In a
headless snapshot the six views look from six bearings while the key stays on one. A
half-day fix, and a large part of the complaint.

The census views are diagrams rather than photographs. In `0101-r38-s1-t15000-side.png` the
tank (400 m², 60 m deep) fits vertically, so the box occupies about 295 of 1,600 pixels.
That leaves 82% of the frame as empty background, at about 13 px per metre.

A 0.3 m body projects to under 4 px, below `MinimumBodyPixels`, and is replaced by a
stamped 5 px square that ignores occlusion (`SnapshotCamera.cs:183-199`). Every dot in the
side, top and iso views is a marker. Correct for a census, useless for video.

Two close views read well. In `r37b-s7-t5000-close.png` and `r37-s1-t15000-close.png` the
carve reads, the guild rim reads, the mottle reads, and the depth-fog separation reads.
...
Two more specifics come out of the pictures. The first is that
`r38-s1-t30000-close.png` is dark partly because it is true: brightness carries reserve,
and round 38's world was starving. The signal is real, and it falls off the bottom of the
display range.

The second is motion. The provenance picture shows "200× requested, 22.64× actual" at 67
bodies. At 22× with dt 0.01 each rendered frame advances about 40 physics steps, and bodies
strobe. Cinematic motion needs a pace near 1×, at which a late second is hours away. That
is why the checkpoints are the real enabler for film.
...
`CristianQiu/Unity-URP-Volumetric-Light` (MIT) and Keijiro Takahashi's `Kino` (Unlicense).
His other repositories follow the pattern, but were not checked one by one. Then Poly Haven
and ambientCG (CC0, no attribution), and IBM Plex (SIL OFL 1.1, in use). Three things are
not available in URP 17, and HDRP has them: volumetric fog, screen-space reflections and
auto-exposure.

Everything else on the wish list ships in the volume system. It has bloom, tonemapping,
depth of field, vignette, colour adjustments, curves, LUT, film grain and motion blur. It
also has lens distortion, chromatic aberration, Panini, white balance, split toning,
lift-gamma-gain, shadows-midtones-highlights, channel mixer and screen-space lens flare. Unity Personal is fine: the threshold is $200,000
...
**R1, the water is black by design**, which protects the dark field and the rim's contrast.
...
green producer against blue-green water is harder to separate. It is paid another way: the
guild reads on the rim and the body, and near the surface the back light (B2) carries the
silhouette. The census views keep the current fog, so nothing in our record changes. The
world's own light model says the top 12 to 18 m is lit, so a black surface layer is further
from the truth than a graded one.

**R2, colour carries meaning on the rim only**, which protects against flat coloured boxes
and against a texture putting hue where it means nothing. Alternative: guild hue also
through transmission (D3), so that a producer glows green when backlit because light passes
through it, and an eater does not. Cost: hue on the face of a backlit body, and two guilds
could be confused at a grazing angle. It is paid another way: transmission is gated on the
lighting geometry rather than painted, and face albedo stays at 0.18. This may be an
extension of the rim rule, and it is the owner's to rule.

**R3, the census views are unchanged in framing**, which protects comparability across
entries. Alternative: a portrait aspect for deep narrow worlds (E4). Cost: a picture in a
later entry is not directly comparable to an earlier one. It is paid another way. Freeze
the current framing as "census v1", stamp the framing version into the label the picture
already carries, and take both for one round as the bridge. A1's grading has the same
property and needs the same versioning note.

**R4, markers stand in for bodies too small to draw**, which protects "where are the
bodies" as a question a picture can answer. Alternative: in film frames only, a soft
additive glow that respects occlusion. Cost: a count read off a film frame under-reports.
It is paid another way. Markers stay in every picture our record cites, and the glow exists
only in the safari's output, where no caption states a count from the image.

**R5, no hue in the chrome**, which protects the instrument's honesty. Alternative: the
film's chrome (title cards, lower thirds, chapter cards) may carry hue. Cost: almost none,
provided the two are never on screen together. It is paid another way: the safari hides the
interface while recording, and that is the boundary. The rule binds the instrument and
...
uploaded, since a prompt is not project data. Then title cards, chapter cards, thumbnails,
...
days are spent building it.

It is not for bodies, the bed, caustics, the snow, per-clade skins from a genome hash, or
anything our record must reproduce. Any project image going up needs the owner's approval
for that instance.
...
and C3 (pending R1). Then D1, D3 (pending R2), D5, E3, F1, F3, F4 and F5. Footage fit for
YouTube: a descent becomes a descent, a portrait separates from its crowd, a producer glows
with the sun behind it. Six to seven days, £0.

**The ambitious version, "a film, not frames"**: the above plus the three specs already
queued, which are the timeline, the checkpoints and the safari director with Cinemachine.
The checkpoints are a rendering prerequisite, since no late second can be filmed at 1×
without them. Add E1 and F6, and D6 if the owner rules for it. Fifteen to eighteen days on
top, £0 in tools.

The one-day pass comes first. Every judgement in the other two is made from pictures, and
the pictures are taken through a broken chain. There is no tonemap, no anti-aliasing, an
8-bit output, and a light pointing the wrong way. It is also the only package whose gain is certain to be
large; the rest is re-assessed from the pictures the first pass produces.

## 6. Open questions for the owner

1. Grading changes how every future picture looks, though not its framing. One choice is
   to freeze the current look as v1, stamp a look version into the label, and grade from
   here on. The other is to keep our pictures ungraded and grade only film frames. The
   first is recommended.

2. Linear colour space costs two days, a full reimport on every worker copy, and a re-tune
   of every dial. The choice is whether correct falloff forever is worth that, or whether
   gamma stays as it is.

3. R1 and R2 ask whether the top ten metres may read as lit water, and whether guild hue
   may enter through backlit translucency. Both are the dark-field rule's edges, and both
   are where the largest "alive" gains sit.

4. D4, drawn eyes: an ornament that says so, or nothing at all.

5. D6, inherited skin genes: the only item that makes a clade visible as a clade, and the
   only one that touches `Evosim.Core`. The choice is whether it comes for the videos or
   after the campaign.
