...
| G1 | **gestation enters**: `gestationBirths` above 0 at 30,000 s in 3 of 3 (the mode flips at 0.08 per reproduction-trait mutation, and a gestating lineage's children carry `gm` 1) | `stats.jsonl`; the birth rows' `gm` |
| G2 | **gestation is not selected out**: the share of birth rows with `gm` 1 in the last 5,000 s is at least the share in the 5,000 s after the floor closes (3,000 to 8,000 s) in 2 of 3, a weak clause since the share starts at 0 and any birth in the last window holds it; the strong reading printed beside it is the gestating lineages' children per dead body against the lump lineages' in the same window | the birth and death rows by `gm` |
...
| F4 | **founders land in their food**: every pool founder's first positions row is within one 1 m layer of the richest snow layer of its column in the dump at or before its birth, in 3 of 3 (the rule's own check; a founder placed by the depth draw would sit anywhere in the top 12 m) | `positions.jsonl` against `fields/*.snow-columns.f32` and the layered snow dump |
| E1 | **the endowment is spent and not hoarded**: a reading, not a clause: the founder rows' `endow` (median and range; the smoke read 12 to 223 J) against the founders' median age at death and the share that bred | `lineage.jsonl` |
...
- **The reader is `scripts/reads/r48-read.py`**, round 47's reader with the new rows and
  ...
  `gestationJoulesHeld`; the layered snow dump), checked on the smoke before the launch.
