...
ARMS = ['r37b-s%d' % i for i in range(1, 6)]
...
hist = ['## Radial histogram, r37b (eight shells of EQUAL AREA, so a uniform disc reads'
...
ang = ['## Azimuthal histogram, r37b (twelve sectors of 30 degrees, equal area; a spread'
...
dep = ['## Depth histogram, r37b (twelve bins of 5 m from the surface to the bed)',
...
drift = ['## Radial drift, r37b: for every body present in two consecutive samples (100 s'
...
# The same drift statistic on round 37, so the bound and the pooled flux are comparable:
# 0094 reported round 37's bin means against zero, and zero is not the right null.
drift.append('')
drift.append('## Control: the same statistic on round 37 (the drag-only tank, 0094)')
drift.append('arm\tr bin (m)\tbody pairs\tmean dr (m per 100 s)\tmedian dr'
             '\tshare moving outward\tfully-mixed bound\tobserved / bound')
