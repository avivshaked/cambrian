...
- **Added mass has been off in every run.** Every config in the repository reads
  `addedMassCoefficient` 0, while DESIGN §5.4 promotes added mass to Milestone 3 on the
  finding that a simplified fluid collapses body-plan diversity. That is now a gotcha in
...
- **DESIGN §7 claimed the config hash covered the timestep and the Unity version.** It never
  did; they are in the manifest. Corrected today.
- **Three instruments** went into HANDOFF's queue: a contract test at the Unity boundary, the
  transactional guard test D052 queued, and round 18 committed as a reference rather than
  living only as a launcher script and five gitignored directories.
...
- **What "food chain" means under D063.** The scorer accepts any body expressing absorptive
  tissue, a mixotroph living mostly on light included, and nothing weighs how much of its
  income is detrital. The question is now noted under D063 for the owner, with the
...
needs. Both were built the same night. Its third finding, that round 28 had three pre-launch
faults, was answered before the launch: the digest on the main arm, the control's replay
checked on the current build (logbook/0070), and the treatment named as the package it is.
...
- **Seven conditions on the predation proposal** are appended to it, so the owner rules on a
  ...
  income.
- **What the review got wrong.** It said round 27 ran at fifteen job worker threads, a
...
and 4 superseded by later rulings. The 10, and the live halves of the 33, are what this
entry and today's edits put into CLAUDE.md, DESIGN.md, DECISIONS.md, HANDOFF.md, README.md,
the primer, the research queue and the predation proposal. The three files are deleted, and
