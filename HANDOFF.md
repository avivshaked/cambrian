...
## The theatre's interface (owner 2026-09-12 night: `design/SPEC.md`, "review and let's discuss")

Pencilled in, not yet ruled. The spec is a build spec for replacing the theatre's IMGUI
text block with a UI Toolkit interface: the identity strip, the census, the warnings, the
transport bar and timeline, the provenance popover, the inspector in three states; the
single-creature mode as a mode of the same screen; charts, the run picker, the lineage view
and the gallery deliberately left for a later pass. Everything lands under
`unity/Assets/Theatre/UI/` and nothing under `Assets/Evosim`, so `simHash` does not move and
no recording is orphaned. The agent's review of 2026-09-12 night: the data contract checks
