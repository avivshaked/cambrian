...
| [0048](logbook/0048-stirring-the-pot.md) | 12 · **running** | D066 roll cells (4 patches, 0.3 m/s, 30 m cells, period 6,000, blink 3,000, fields advected) — arm A; + excretion 0.01 with the fixed matter term non-excretable — arm B | Two mid-round dose changes on pre-registered readings: cell 60 → 30 (Sverdrup: `r12a-s1` never founded, stirred into the dark); speed 0.1 → 0.3 (the roll's surface dead zone). **S1 falsified at both speeds by mechanism**: a roll that stops at 30 m is a trapdoor — the down leg drops remains below the cell, where the 0.02 m/s sink takes them for good; surface detritus 0.24–0.53 J/m³, deep 12–15. Arm A stopped at one seed (`r12x-s1`); arm B's five seeds run (`r12y-s1..5`). **First inherited absorptive since round 8** in `r12y-s3` at t≈8,900 (a line of 2–3 individuals, alive 3,500 s on). Proposal to the owner: round 13 = arm B + `EVOSIM_SINK`/`EVOSIM_MATTER_SINK` 0.002 (Stokes for remains; knobs added, defaults unchanged) |
