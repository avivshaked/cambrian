# Round 41d (D099, 2026-09-19; logbook/0108): round 41c's world with the silhouette cap on and the
# joint's price lifted back. Round 41c's three seeds were stopped at 3,300 to 3,800 s as futile on the
# same contact explosion as 41b's, and the cause was inside bodies: a terminal-only self-edge grew a
# knot of nine or sixteen parts, every part overlapping every other, that earned sixteen parts' light
# from one point because income and shade were the sum of the parts' areas and a body never shaded
# itself (0107's last section; scripts/overlap). EVOSIM_SILHOUETTE 1 caps a body's lit area at its
# convex hull's silhouette (RunConfig.LightSilhouetteCap; logbook/specs/silhouette-spec.md), and the
# build makes a terminal-only edge keep the recursive limit. The joint was never the cause, so
# EVOSIM_IDLE goes back to 0.0001, round 41b's, and round 34's free-joint question is asked here.
# Everything else is 41c's. Header token 'silhouette on'.
#   ./rounds/launch-r41d.ps1 -Seed 1 -Worker 2 -ExpectSimHash <hash>                        # r41d-s1
#   ./rounds/launch-r41d.ps1 -Seed 1 -Worker 2 -Seconds 5000 -Name r41dscreen-s1 -Dt 0.02   # the screen
# Verify the header: everything launch-r41c.ps1 lists, 'silhouette on', 'idle 0.0001 W/N.m'; the
# config's lightSilhouetteCap true.
# Round 41b (2026-09-18, night; logbook/0107's second launch): round 41 with one dial moved. Round 41's
# three seeds were stopped at 7,000 to 9,600 s as futile: the marine snow nobody ate stood at half the
# 3,000-unit budget in every seed, the leaves uptake-bound on 60 to 90% of their steps, because the
# remineralisation rate was sized for a fifth of 11,000 units. EVOSIM_REMIN 0.002 (half-life 350 s)
# puts that stock back near a fifth of 3,000. Everything else is round 41's, and the base round of
# the one-substance economy is this launch.
...
# ASCII only.
#   ./rounds/launch-r41c.ps1 -Seed 1 -Worker 2 -ExpectSimHash <hash>                        # r41-s1
#   ./rounds/launch-r41c.ps1 -Seed 1 -Worker 2 -Seconds 5000 -Name r41smoke-s1 -Dt 0.02     # the screen
...
param(
    ...
    [float]$Idle = 0.0001,   # 0.02 in round 41c on a diagnosis the round overturned; see the head of this file
    [int]$Silhouette = 1,    # D099: EVOSIM_SILHOUETTE, 1 from round 41d, 0 replays every earlier world
    ...
    [float]$UptakeK = 0.3,
    [float]$UptakeKs = 0.05,
    [float]$Remin = 0.002,   # 0.0005 in round 41; see the head of this file
    [float]$Handling = 0.1,
    [float]$ReserveCap = 0,
    [float]$MarginMin = 0,
    [float]$MarginMax = 600,
...
$s = @{
    ...
    EVOSIM_IDLE = $Idle; EVOSIM_SILHOUETTE = $Silhouette
...
$name = if ($Name -ne '') { $Name } else { "r41d-s$Seed" }
$extra = @{}
