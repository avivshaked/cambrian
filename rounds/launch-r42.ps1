# Round 42: round 41e's world on half the matter, 1,500 units (the owner's ruling of 2026-09-20:
# the crowd is the budget's, so half the units is about half the bodies and half the wall), on
# the build with lever 1 and the trace restricted to jointed bodies (identity kept, merged
# bfc0993), and with each arm pinned to two fast cores of its own by worker number
# (run-arm.ps1 -AffinityMask; harness-profile-spec.md section 9). Everything else is round
# 41e's (rounds/launch-r41e.ps1), whose parameters this file keeps below.
#
#   ./rounds/launch-r42.ps1 -Seed 1 -Worker 2 -ExpectSimHash <hash>                        # r42-s1
#   ./rounds/launch-r42.ps1 -Seed 1 -Worker 6 -Seconds 300 -Name r42smoke                  # the smoke
#
param(
    ...
    [float]$MatterBudget = 1500,   # 3,000 in rounds 41 to 41e
    ...
    [float]$WaterHold = 0.5,   # EVOSIM_WATER_HOLD, seconds; 0 samples the water at every link every step
    [float]$SelfOverlap = 0.1, # EVOSIM_SELF_OVERLAP, the depth fraction; 0 admits a body born inside itself
...
$s = @{
    ...
    EVOSIM_WATER_HOLD = $WaterHold; EVOSIM_SELF_OVERLAP = $SelfOverlap
...
$name = if ($Name -ne '') { $Name } else { "r42-s$Seed" }
$extra = @{}
# Two fast cores an arm: worker 2 on logical 2-5, 3 on 6-9, 4 on 10-13; any other worker unpinned.
$masks = @{ 2 = 0x003C; 3 = 0x03C0; 4 = 0x3C00 }
if ($masks.ContainsKey($Worker)) { $extra.AffinityMask = $masks[$Worker] }
