# Round 37 (fable-propose-box.md, the ruled path's second round): round 36's world in a box 10 by
# 10 by 60 m, the four patches laid two by two instead of four in a row, at the same area, depth,
# cells, dose, dispersal radius and current. Nothing else moves. The one knob is
# EVOSIM_PATCHES_ACROSS (RunConfig.PatchesAcross, default 1, at which the world is round 36's to
# the bit: boxdig-old against boxdig-new, 2026-09-11, identical over 31 digest steps). ASCII only.
...
# Verify the header: everything launch-r36.ps1 lists, and 'space shared 2x2x5 m, depth 60' where
# every round before reads 'shared 4x1x5 m' (or '4x5x5 m' under the builds before this one).
# A build that does not know the knob ignores it silently and runs the old box; the header is the
# only proof the layout arrived.
param(
    ...
    [float]$LinkPhoto = 0.5,
    # fable-propose-box.md: patches across z. 1 is every round on file; 2 lays four patches two by two.
    [int]$PatchesAcross = 2,
...
$s = @{
    ...
    EVOSIM_PATCHES_ACROSS = $PatchesAcross
