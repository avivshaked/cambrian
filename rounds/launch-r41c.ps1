# Round 41c (2026-09-19; logbook/0107's third launch): round 41b with the joint priced. Round 41b's
# three seeds were stopped at 5,600 to 7,900 s as futile when the physics cost doubled every thousand
# seconds on contact pairs, a thousandfold round 40's, tracking the jointed count across every run on
# file: under one substance the joint had become free (tissue 0.38 J, growth off the field, the link
# earning half a leaf's light, the idle charge 1e-4). EVOSIM_IDLE 0.02 is round 33's price, the
# standing charge per newton-metre of capacity, restored. The build also carries the contact
# instrument (logbook/specs/contact-instrument-spec.md), so simHash moves. Everything else is 41b's.
...
param(
    ...
    [float]$Idle = 0.02,   # 0.0001 in rounds 34 to 41b; see the head of this file
...
$name = if ($Name -ne '') { $Name } else { "r41c-s$Seed" }
$extra = @{}
