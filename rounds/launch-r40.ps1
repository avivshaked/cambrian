# Round 40 (D096, fable-propose-light.md absorbed on 2026-09-18): round 39's world with the light's
# reach halved. Everything is launch-r39.ps1 verbatim -- the shaped bed at D093's size, the streams
# with the acceleration force, the corpses, the prices, 11,000 units of matter -- plus one dial:
# EVOSIM_LIGHT_REACH, the depth over which the irradiance falls by a factor of e (LightModel's
# attenuation depth, config light.attenuationDepth). 12 m is every round before this one and the
# launcher's default; 6 m is D096's ruling. Header token 'light reach 6 m'.
#
# Why: round 39 read the shelf as never in the light (0105): at 12 m the leaves float at 16 to 20 m,
# the shallow arc at 30 m sees a third of the light they do, and 45 m of the water is lit enough to
# live in, so the world is one leaf crowd from top to bottom. At 6 m a lone leaf breeds only in the
# top ten metres (the ledger sweep in D096), so the round asks whether the leaves rise into a band,
# whether shading becomes a selected quantity there, and whether the dark water below becomes the
# eaters' column.
# ASCII only.
#   ./rounds/launch-r40.ps1 -Seed 1 -Worker 2 -ExpectSimHash <hash>                        # r40-s1
#   ./rounds/launch-r40.ps1 -Seed 3 -Worker 5 -Seconds 600 -Name r40smoke -Dt 0.02         # the smoke
#   ./rounds/launch-r40.ps1 -Seed 1 -Worker 2 -LightReach 12 -Name r40ctl-s1               # round 39's light, new build
# Verify the header: everything launch-r39.ps1 lists, and 'light reach 6 m'; the config's
# light.attenuationDepth reads 6. Read 'depth m', 'shade %', 'photo inh' and 'inherit' first, and
# the leaves' and the eaters' median depths from positions.jsonl.
param(
...
$name = if ($Name -ne '') { $Name } else { "r40-s$Seed" }
$extra = @{}
