# Round 36 (logbook/0089): round 34's free-joint world with the link part earning light at half a
# leaf's efficiency. Nothing else moves. The one knob is EVOSIM_LINK_PHOTO (LinkCell.PhotosyntheticEfficiency,
# DESIGN 5A.10, unmeasured, 0 in every round before), so a hinge sits on a producer instead of on
# dead weight. Everything else is launch-r34.ps1 verbatim. ASCII only.
#   ./rounds/launch-r36.ps1 -Seed 1 -Worker 2 -ExpectSimHash <hash>                       # r36-s1
#   ./rounds/launch-r36.ps1 -Seed 1 -Worker 7 -Seconds 600 -Name r36smoke -Dt 0.02        # the smoke
# Verify the header: everything launch-r34.ps1 lists ('idle 0.0001 W/N', 'work x0', 'neuron 0 W +
# 0 W/input', 'dispersal=5 m', 'current 0.1 m/s transport'), plus 'linkPhoto 0.5'. Read 'jnt inh'
# against 'alive' every 1,000 s against round 34's same seed.
param(
...
$name = if ($Name -ne '') { $Name } else { "r36-s$Seed" }
$extra = @{}
