# Round 35 (2026-09-10): round 33's world with newborns dispersed over a disc instead of set down
# touching their parents. Nothing else moves. On 2026-09-10 the theatre showed round 33 seed 3 as
# two vertical ribbons about a metre wide: a newborn is placed against its parent (D077 rule 5),
# the rolling current returns a body to where it found it (D059), and nothing else moves a sitter
# sideways, so a clade is a column packed around the spot its founder landed on for the whole run.
# Since the grid (D086) a mouth drains the one cell it stands in, so a ribbon drains the same
# handful of cells for thirty thousand seconds. The 5 m disc is a first cut, for the owner to rule
# on. Everything else is launch-r33.ps1 verbatim. ASCII only.
#   ./rounds/launch-r35.ps1 -Seed 1 -Worker 2 -ExpectSimHash <hash>                       # r35-s1
#   ./rounds/launch-r35.ps1 -Seed 3 -Worker 7 -Seconds 600 -Name r35smoke -Dt 0.02 -CurrentMode rolls
#   ./rounds/launch-r35.ps1 -Seed 3 -Worker 7 -Seconds 600 -Name r35tsmoke3 -Dt 0.02
# The default is now -CurrentMode Transport (the owner's ruling of 2026-09-10), so -CurrentMode
# rolls is what reproduces round 35 as launched. Read the header's 'current <speed> m/s <mode>'
# token to see which world ran. A transport world splits its advection step when the fastest water
# would cross more than half a cell in one; the -Current note below has the arithmetic.
# Verify the header: everything launch-r33.ps1 lists, plus 'dispersal=5 m' immediately after the
# 'space' token, and in the table the three new columns 'cols', 'cols abs' and 'x sd' at the end
# of the base columns, before the per-patch ones. Read 'cols' against 'alive': a thousand
# creatures standing on seventeen columns of a hundred is the ribbon world this round exists to
# break. The variable is EVOSIM_OFFSPRING_DISPERSAL and not EVOSIM_DISPERSAL -- that name belongs
# to D061's retired patch lottery, and one name for two knobs sets both.
param(
...
$name = if ($Name -ne '') { $Name } else { "r35-s$Seed" }
$extra = @{}
