# Round 46's environment block: round 45's world (rounds/env-r45.ps1: the 22,000 m2 tank at
# 45 m, 15,000 units as islands, the matter grid stirred at 0.02, the mouth's tunables) plus
# the seven rules ruled on fable-propose-round-46.md and absorbed as D110 to D116, and the
# beach. The shape scripts/run-farm.ps1 -Launcher wants; overrides go on the command line.
#
# What changes from round 45, each with its spec under logbook/specs/:
# - light by exposure (D110, light-exposure-spec.md): a part's lit area is its area facing the
#   light, so a leaf on edge earns nothing and a flat one twice the orientation average.
# - the buoyancy offset (D111, buoyancy-offset-spec.md): a genome field that moves a part's
#   centre of buoyancy along its thinnest axis, priced at 0.02 W per m3 at full offset.
# - the founding trickle (D115, founding-trickle-spec.md): one founder every 30 s on average
#   for the whole run after the floor closes, booked as influx; an empty world ends only when
#   no founders are arriving.
# - founders follow their food (D116, the same spec): a stomach lands where the snow is, a
#   leaf where the dissolved matter is; D109's follow-matter rule is off, the two are refused
#   together.
# - the support cost (D113, support-cost-spec.md): 0.1 W per m2 of lit area per m2 of its
#   squared distance from the root, the price that stops the fan.
# - contact per part (D114, per-part-contact-spec.md): a sphere on every part.
# - the snow's life (D112, stomach-screens.md): remineralisation at a quarter of round 45's
#   rate, 0.0005 /s, so the larder under the plants is about twice as thick.
# - the beach (beach-spec.md): the bed tilted 96 m across the tank with a shore at 1 m and
#   the current fading over 15 m of depth into it, so a shelf within 12 m of the surface
#   holds about a tenth of the floor in the light.
#
# Everything else is round 45's, including the checkpoint every 2,500 s (a recording setting).
@{
