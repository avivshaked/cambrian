...
# mean depth and far below the lit band, so this round reads pockets and not light); and
# EVOSIM_BED_SCALE, the largest band's wavelength (0 = a third of the diameter, 7.52 m here).
# Relief 0 and tilt 0 is round 38's world to the bit, which is what the digest pair checks.
#
# Why: a uniformly dilute world is a desert (0100), and the shaped floor is the concentrator the
# design has wanted from the start -- hollows where the water slows and the sediment gathers, a
# ridge where it quickens, a gradient so the tank is not one depth. The round asks whether the
# floor makes places: floor low % against floor J, det cv against round 38's, where the eaters
# sit against where the floor is low.
# ASCII only.
#   ./rounds/launch-r39.ps1 -Seed 1 -Worker 2 -ExpectSimHash <hash>                        # r39-s1
#   ./rounds/launch-r39.ps1 -Seed 3 -Worker 5 -Seconds 600 -Name r39smoke -Dt 0.02         # the smoke
#   ./rounds/launch-r39.ps1 -Seed 3 -Worker 5 -Seconds 600 -Dt 0.02 -DigestEvery 1000 -BedRelief 0 -BedTilt 0 -Name tankdig-bed
# Verify the header: everything launch-r38.ps1 lists, and the space token reading
#   'space tank r=11.28 m (400 m2), depth 60, wall, bed relief 1 m tilt 6 m scale 7.52 m (hollows n, ridges n, range x.xx m, steepest nn deg bands nn deg, bound binds)'
# and, at relief 0 and tilt 0, exactly round 38's '..., wall, bed'. Read 'floor low %' and
# 'floor J' (a dash on a flat bed) beside 'det cv', and 'diverged' for the bed's own guard.
param(
...
$name = if ($Name -ne '') { $Name } else { "r39-s$Seed" }
$extra = @{}
