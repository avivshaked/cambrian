# Round 41e: round 41d's world with the two cheapenings the owner ruled on 2026-09-19
# (logbook/specs/harness-profile-spec.md §7; logbook/specs/cheapening-spec.md). EVOSIM_WATER_HOLD 0.5
# samples the water once a body at its root and holds it for a metabolic step, in place of every
# link every physics step (FluidConfig.WaterHoldSeconds; 0 replays every earlier world).
# EVOSIM_SELF_OVERLAP 0.1 makes a body born with two non-adjacent parts overlapping deeper than a
# tenth of the smaller part's thinnest half-extent a counted stillbirth (RunConfig.
# SelfOverlapDepthFraction; 0 replays every earlier world; the physics of self-collision stays on).
# Everything else is round 41d's (rounds/launch-r41d.ps1), whose header this file keeps below.
#
#   ./rounds/launch-r41e.ps1 -Seed 1 -Worker 2 -ExpectSimHash <hash>                        # r41e-s1
#   ./rounds/launch-r41e.ps1 -Seed 1 -Worker 6 -Seconds 300 -Name r41esmoke -Dt 0.02          # the smoke
#
param(
...
$name = if ($Name -ne '') { $Name } else { "r41e-s$Seed" }
$extra = @{}
