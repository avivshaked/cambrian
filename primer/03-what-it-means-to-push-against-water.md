...
nothing to swim in. [Piece 02](02-from-a-tree-of-boxes-to-something-that-moves.md) ended
with bodies that move, with joints turning and parts flailing, and no way for any of that to
become travel.
...
power stroke and compliant on the return. An oar is feathered on the way back. A jellyfish
contracts fast and refills slowly.

So the fluid model has one job it cannot fail at. **A surface moving broadside must generate
much more force than the same surface moving edge-on.** Get that wrong and no rhythm
produces net travel, whatever the controller does. Everything oscillates in place forever.
...
Density, a drag coefficient, the patch's area, and the square of the speed at which that
patch is moving *along its own normal*. Only patches advancing into the water contribute.
Pressure drag acts on leading surfaces, and counting trailing ones would both double the
force and cancel the asymmetry the whole thing depends on.

Summing over the surface rather than using one whole-body number is what makes the
broadside/edge-on difference automatic. A thin plate presents a large area along one axis
and a sliver along the others. The same formula gives ten times the force in one direction
as the other, with nothing special written for it.

[C18 §2.2, p.5] arrived at the same scheme independently, as a *"simplified mesh-based
quadratic drag model"* projecting each facet's speed along its normal, with `Cd = 1.5`. That
is reassuring about the approach. It also means everything that paper reports about the
model's limitations applies here directly, which turns out to matter a great deal, and there
is more on it below.
...
*translating*. You also get **zero** drag for a box *spinning about one of its own axes*.
...
Now recall what a limb does. A limb attached to a joint rotates. That is the entire motion.
A paddle is a surface turning about its root, and under centre-sampling most of its
interaction with the water does not exist at all.

The fix is to integrate over the surface instead of sampling a point, which means dividing
it into panels and summing. Points away from the rotation axis *do* have normal-direction
velocity, and they produce the torque that resists a spinning limb. Two panels per axis, so
four per face and twenty-four per box, is the cheapest version that works.
...
Parts are boxes, spheres or capsules. That sounds like variety for its own sake, and it is
something else. Once drag is summed over a surface, the shape of that surface *is* the set
of strokes available to the creature.

So here it is, measured. Take one set of half-extents, flat, 1.2 m across and 0.1 m thick,
and push each shape through water broadside, then edge-on:
...
The box is the paddle. It is the only one of the three that can be *flat*, since a sphere's
radius is one number and a capsule is round in cross-section. Flatness is where a factor of
twelve comes from.

And the sphere is the interesting entry, because 1.01× is not an approximation of a small
effect. It is the statement that **a sphere cannot paddle at all.** Every orientation
presents the same area, so turning it buys no fast stroke and no slow return. Sweeping one
sideways produces drag along its path and nothing perpendicular to it. A sphere may be a
head, a float or a body that carries other parts, and it is not for thrust.
...
summing pressure over a surface, in the same way the broadside/edge-on difference does. A
test asserts it, so if it ever stops being true, something upstream has broken.

The capsule sits in between and is the useful middle: round in cross-section like a sphere,
but elongated, so it has one axis of anisotropy and no more. Broadside it is a long side;
end-on it is a small circle. That is a limb.

None of which prejudges what evolution should build out of them. It only means the three
shapes are different offers, rather than three ways of drawing the same part.
...
threshold. It is a physical law, and it cannot be satisfied by an impressive-looking
failure.
...
this one belongs to, against real hydrodynamics on an evolved creature. The two disagreed
about its **direction of travel**, which is a sign flip rather than a magnitude error. And
[U07 §2, p.3] notes the gait the search had found was *"unusual, and we have never observed
such motion in natural phenomena."* The search had not discovered swimming. It had
discovered the model's error and exploited it, in published work, in this exact environment.
...
body are absent. No squid, because jetting cannot pay off without added mass. What is left
is a gallery of similar blobs. Since the entire point here is creatures that are interesting
to look at, the cheap model fails hardest at the goal it appeared safe for.

That inversion is worth sitting with. The usual assumption is that physical accuracy is the
scientist's concern and visual appeal is the artist's. A simplified model then trades the
first for cheapness while leaving the second alone. It does not. In an evolutionary system,
the physics *is* the space of available strategies, and a smaller space of strategies means
a smaller space of bodies worth having.

### How it is applied

Added mass is a force proportional to a body's own acceleration, which makes it an implicit
term. Integrate it explicitly and it feeds back on itself. The simulation then diverges as
the added mass approaches the real mass, which for a neutrally buoyant creature is where it
sits.
...
| seed | parts | DOF (degrees of freedom, the joints' axes) | displacement | speed | J per metre |
|---|---|---|---|---|---|
...
the best travels fifty times as far as the worst that moves at all, on the same controller
and the same physics. One does not move.

That spread is the raw material a search needs.

The last column is a warning label as much as a measurement, and here is why. Energy alone
says nothing without a performance level to divide it by, as [C18 §3, p.17] puts it.
...
Seed 8 does not move at all, and cannot be scored on this axis without dividing by zero.
That is the right answer rather than an inconvenience.

An earlier draft of this piece reported the same experiment without the energy column. It
also did not know that most of what these creatures spend is destroyed against their own
joint limits rather than delivered to the water. Both of those are recent
...
- The argument that physics-as-strategy-space explains why a simplified model costs variety
  rather than only accuracy. This is a reading of [C18]'s result rather than a claim [C18]
  makes.
- **That a sphere cannot paddle.** No source says this; it is a property of *this*
  implementation, measured in the table above and asserted by a test. It follows from
  summing pressure over an isotropic surface, and it would not survive a fluid model that
  captured vortex shedding. Per [C18 §4, p.28], this one does not.
- The reading of "a creature that does nothing is perfectly efficient" as the reason the
  per-metre column is the one to look at. The general point about energy needing a
  performance level is [C18]'s, and the illustration from seed 5 is ours.
...
| [`FluidModelTests.cs`](../src/Evosim.Core.Tests/FluidModelTests.cs) | the force tests, including the energy law |
| [`ShapeTests.cs`](../src/Evosim.Core.Tests/ShapeTests.cs) | the isotropy and flat-paddle measurements above |
